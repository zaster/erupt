...
public class EruptRowOperationConfig implements EruptRecordOperate.DynamicConfig {
    ...
    private RowOperation findRowOperation(EruptModel eruptModel) {
        String code = request.getServletPath().replace("/importx", "")
                .split(EruptDataController.OPERATOR_PATH_STR + "/")[1];
        return Arrays.stream(eruptModel.getErupt().rowOperation()).filter(operation -> operation.code().equals(code))
                .findFirst()
